...
client.on('ready', () => {
  console.clear();
  client.setInterval(() => {var act = [{"text": "Playing with colors! :)","type": "PLAYING"},{"text": "By Hideaki#0136","type": "WATCHING"},];try {const activity = act[Math.floor(Math.random() * act.length)];client.user.setActivity(activity.text, { type: activity.type });} catch (err) {return;}}, 15000);
